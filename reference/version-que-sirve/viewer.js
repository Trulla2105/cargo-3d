...
function setupManualPlacement(dom) {
  ...
  window.addEventListener('keydown', e => {
    if (manualDragBox) {
      if (e.key === 'r' || e.key === 'R') { manualDragOri = rotIdx(manualDragOri, 'y'); e.preventDefault(); }
      if (e.key === 't' || e.key === 'T') { manualDragOri = rotIdx(manualDragOri, 'z'); e.preventDefault(); }
      if (e.key === 'y' || e.key === 'Y') { manualDragOri = rotIdx(manualDragOri, 'x'); e.preventDefault(); }
      return;
    }
